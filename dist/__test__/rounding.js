...
var round = require('../custom_modules/rounding.js');
describe('round', function () {
    describe('sumWithPrecision', function () {
        ...
        it('should return 100 when supplied with array that totals 100', function () {
            for (var i = 0; i < positiveTestCases.length; i++) {
                var res = round.sumWithPrecision(positiveTestCases[i]);
    ...
    describe('toPercent', function () {
        ...
        it('should return array with correct amount of decimal places when precision argument passed', function () {
            for (var i = 0; i < testCases.length; i++) {
                var test = round.toPercent(testCases[i], round.sumWithPrecision(testCases[i]), 2);
                for (var j = 0; j < test.length; j++) {
                    var split = test[j].toString().split('.');
                    if (split.length == 2)
                        expect(split[1].length).to.be.lte(2);
                }
            }
        });
    });
