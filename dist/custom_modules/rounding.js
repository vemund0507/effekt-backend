...
module.exports = {
    toPercent: function (input, total, decimals) {
        ...
        for (var i = 0; i < input.length; i++) {
            ...
            if (decimals)
                result[i] = setDecimals(result[i], decimals);
            if (result[i].greaterThan(0))
                lastWithValue = i;
        }
        if (lastWithValue) {
    ...
    sumWithPrecision: function (input) {
        ...
        for (var i = 0; i < input.length; i++) {
            var toAdd = void 0;
            var isDecimal = input[i] instanceof decimal;
            var type = typeof input[i];
            if (type === "string")
                toAdd = new decimal(input[i]);
            else if (isDecimal || type === "number")
                toAdd = input[i];
            else
                throw new Error("Invalid input in sumWithPrecision method, use either string, number or decimal");
