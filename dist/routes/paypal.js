var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var express = require('express');
var router = express.Router();
var DAO = require('../custom_modules/DAO.js');
var mail = require('../custom_modules/mail.js');
var bodyParser = require('body-parser');
var urlEncodeParser = bodyParser.urlencoded({ extended: false });
var request = require('request-promise-native');
var config = require('../config');
var websocketsHandler;
require('request').debug = true;
router.post("/ipn", urlEncodeParser, function (req, res, next) { return __awaiter(_this, void 0, void 0, function () {
    var responseBody, paypalCustomData, KID, wsClientID, sum, paymentDate, transactionID, paymentStatus, verification, ex_1, donationID, ex_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                res.status(200).send();
                responseBody = req.body;
                responseBody = Object.assign({ cmd: "_notify-validate" }, responseBody);
                paypalCustomData = req.body.custom.split("|");
                KID = paypalCustomData[0];
                wsClientID = paypalCustomData[1];
                sum = parseFloat(req.body.mc_gross);
                paymentDate = new Date(req.body.payment_date);
                transactionID = req.body.txn_id;
                paymentStatus = req.body.payment_status;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, request.post("https://ipnpb.paypal.com/cgi-bin/webscr", {
                        encoding: 'UTF-8',
                        headers: {
                            'User-Agent': 'Stiftelsen Effekt IPN Script - Node'
                        },
                        form: responseBody
                    })];
            case 2:
                verification = _a.sent();
                return [3 /*break*/, 4];
            case 3:
                ex_1 = _a.sent();
                console.error("Failed to send paypal verification postback for KID: " + KID);
                return [3 /*break*/, 4];
            case 4:
                if (sum < 0)
                    return [2 /*return*/, false]; //Refunded donation. Might want to automate this aswell.
                if (!(verification == "VERIFIED" && paymentStatus === "Completed")) return [3 /*break*/, 9];
                _a.label = 5;
            case 5:
                _a.trys.push([5, 7, , 8]);
                return [4 /*yield*/, DAO.donations.add(KID, 3, sum, paymentDate, transactionID)];
            case 6:
                donationID = _a.sent();
                return [3 /*break*/, 8];
            case 7:
                ex_2 = _a.sent();
                console.error("Failed to update DB for paypal donation with KID: " + KID);
                console.error(ex_2);
                return [3 /*break*/, 8];
            case 8:
                if (config.env === 'production')
                    mail.sendDonationReciept(donationID);
                websocketsHandler.send(wsClientID, "PAYPAL_VERIFIED");
                return [3 /*break*/, 10];
            case 9:
                if (req.body.txn_type == "subscr_signup") {
                    //Subscribtion IPN, ignore
                }
                else {
                    websocketsHandler.send(wsClientID, "PAYPAL_ERROR");
                }
                _a.label = 10;
            case 10: return [2 /*return*/];
        }
    });
}); });
module.exports = function (wsHandler) {
    websocketsHandler = wsHandler;
    return router;
};
